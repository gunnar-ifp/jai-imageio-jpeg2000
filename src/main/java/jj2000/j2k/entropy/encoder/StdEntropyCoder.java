    ...
    implements StdEntropyCoderOptions
{
     ...
     */
    private static final int RLC_MASK_R1R2 = STATE_SIG_R1 | STATE_SIG_R2 |
    ...
    static {
        ...
        for (i = 0; i < (1 << (MSE_LKP_BITS - 1)); i++) {
            ...
            deltaMSE = val * val;
            FS_LOSSLESS[i] = (int)Math.floor(deltaMSE *
                (1 << MSE_LKP_FRAC_BITS) + 0.5);
            val -= 1.5;
            deltaMSE -= val * val;
            FS_LOSSY[i] = (int)Math.floor(deltaMSE *
                (1 << MSE_LKP_FRAC_BITS) + 0.5);
        }
        ...
        for (i = 0; i < (1 << MSE_LKP_BITS); i++) {
            ...
            FM_LOSSLESS[i] = (int)Math.floor(deltaMSE *
                (1 << MSE_LKP_FRAC_BITS) + 0.5);
            val -= (i < (1 << (MSE_LKP_BITS - 1))) ? 0.5 : 1.5;
            deltaMSE -= val * val;
            FM_LOSSY[i] = (int)Math.floor(deltaMSE *
                (1 << MSE_LKP_FRAC_BITS) + 0.5);
        }
    }
        ...
        int lcType, int tType)
    {
        ...
        ccb.selectConvexHull(ratebuf, distbuf,
            (options & (OPT_BYPASS | OPT_TERM_PASS)) != 0 ? istermbuf : null, npass, rev);
        ...
        int options)
    {
        ...
        for (s = nstripes - 1; s >= 0; s--, sk += kstep, sj += jstep) {
            sheight = (s != 0) ? STRIPE_HEIGHT : srcblk.h - (nstripes - 1) * STRIPE_HEIGHT;
            stopsk = sk + srcblk.w;
                    ...
                    if ((csj & (STATE_SIG_R1 | STATE_NZ_CTXT_R1)) == STATE_NZ_CTXT_R1) {
                        ...
                        if ((symbuf[nsym++] = (data[k] & mask) >>> bp) != 0) {
                            ...
                            if (sym != 0) {
                                ...
                                state[j + 1] |= STATE_NZ_CTXT_R1 | STATE_NZ_CTXT_R2 |
    ...
    public void initTileComp(int nt, int nc)
    {
        ...
        for (int t = 0; t < nt; t++) {
            for (int c = 0; c < nc; c++) {
                ...
                // Bypass coding mode ?
                if (((String)bms.getTileCompVal(t, c)).equalsIgnoreCase("true")) {
                    opts[t][c] |= OPT_BYPASS;
                }
                // MQ reset after each coding pass ?
                if (((String)mqrs.getTileCompVal(t, c)).equalsIgnoreCase("true")) {
                    opts[t][c] |= OPT_RESET_MQ;
                }
                // MQ termination after each arithmetically coded coding pass ?
                if (((String)rts.getTileCompVal(t, c)).equalsIgnoreCase("true")) {
                    opts[t][c] |= OPT_TERM_PASS;
                }
                // Vertically stripe-causal context mode ?
                if (((String)css.getTileCompVal(t, c)).equalsIgnoreCase("true")) {
                    opts[t][c] |= OPT_VERT_STR_CAUSAL;
                }
                // Error resilience segmentation symbol insertion ?
                if (((String)sss.getTileCompVal(t, c)).equalsIgnoreCase("true")) {
                ...
                if (termType.equalsIgnoreCase("easy")) {
                    ...
                    if ((opts[t][c] & (OPT_TERM_PASS | OPT_BYPASS)) == 0) {
                        FacilityManager.getMsgLogger().printmsg(MsgLogger.INFO, "Using error resilient MQ" +
