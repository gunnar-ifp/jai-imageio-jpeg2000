 ...
 * (read in FileBitstreamReaderAgent).</li>
    ...
    StdEntropyCoderOptions
{
    ...
    private void readCRG(DataInputStream ehs) throws IOException
    {
        ...
        FacilityManager.getMsgLogger().printmsg(MsgLogger.WARNING, "Information in CRG marker segment " +
        ...
        int tpIdx) throws IOException
    {
        ...
        if (qType == SQCX_NO_QUANTIZATION) {
            int maxrl = (mainh ? ((Integer)decSpec.dls.getDefault()).intValue() : ((Integer)decSpec.dls.getTileDef(tileIdx)).intValue());
            int i, j, rl;
            int minb, maxb, hpd;
            int tmp;
            ...
            int maxrl = (qType == SQCX_SCALAR_DERIVED) ? 0
                : (mainh ? ((Integer)decSpec.dls.getDefault()).intValue() : ((Integer)decSpec.dls.getTileDef(tileIdx)).intValue());
            int i, j, rl;
            int minb, maxb, hpd;
            int tmp;
            ...
            for (rl = 0; rl <= maxrl; rl++) { // Loop on resolution levels
                ...
                for (j = minb; j < maxb; j++) {
                    ...
                    nStep[rl][j] = (-1f - ((float)(tmp & 0x07ff)) / (1 << 11)) /
                        (-1 << exp[rl][j]);
                }
            } // end for rl
        ...
        int tpIdx) throws IOException
    {
        ...
        if (qType == SQCX_NO_QUANTIZATION) {
            int maxrl = (mainh ? ((Integer)decSpec.dls.getCompDef(cComp)).intValue()
                : ((Integer)decSpec.dls.getTileCompVal(tileIdx, cComp)).intValue());
            int i, j, rl;
            int minb, maxb, hpd;
            ...
            int maxrl = (qType == SQCX_SCALAR_DERIVED) ? 0 : (mainh ? ((Integer)decSpec.dls.getCompDef(cComp)).intValue()
                : ((Integer)decSpec.dls.getTileCompVal(tileIdx, cComp)).intValue());
            int i, j, rl;
            int minb, maxb, hpd;
            ...
            for (rl = 0; rl <= maxrl; rl++) { // Loop on resolution levels
                ...
                for (j = minb; j < maxb; j++) {
                    ...
                    // bits for the exponent, otherwise (-1<<exp) might
                    // overflow (the - is used to be able to represent 2**31)
                    nStepC[rl][j] = (-1f - ((float)(tmp & 0x07ff)) / (1 << 11)) /
                        (-1 << expC[rl][j]);
                }
            } // end for rl
        } // end if (qType != SQCX_NO_QUANTIZATION)
        ...
        int tpIdx) throws IOException
    {
        ...
        if ((cstyle & (SCOX_HOR_CB_PART | SCOX_VER_CB_PART)) != 0) {
            FacilityManager.getMsgLogger().printmsg(MsgLogger.WARNING, "Code-block partition origin " +
        ...
        if ((cblk[0].intValue() * cblk[1].intValue()) > StdEntropyCoderOptions.MAX_CB_AREA) {
        ...
        int tpIdx) throws IOException
    {
        ...
        // Read component
        ms.crgn = comp = (nComp < 257) ? ehs.readUnsignedByte() : ehs.readUnsignedShort();
        if (comp >= nComp) {
            throw new CorruptedCodestreamException("Invalid component " +
                "index in RGN marker" +
                comp);
        ...
        throws IOException
    {
        ...
        switch (marker) {
            ...
            case PLM:
                ...
                FacilityManager.getMsgLogger().printmsg(MsgLogger.WARNING, "PLM marker segment found but " +
            ...
            default:
                htKey = "UNKNOWN";
                FacilityManager.getMsgLogger().printmsg(MsgLogger.WARNING, "Non recognized marker segment (0x" +
        ...
        throws IOException
    {
        ...
        switch (marker) {
            ...
            case PLT:
                ...
                FacilityManager.getMsgLogger().printmsg(MsgLogger.WARNING, "PLT marker segment found but " +
                ...
                for (t = nTiles - 1; t >= 0; t--) {
                    for (tp = 0; tp < nTileParts[t]; tp++) {
                        for (i = 0; i < nPPTMarkSeg[t][tp]; i++) {
                            pkdPktHeaders[t].write(tilePartPkdPktHeaders[t][tp][i]);
                        }
                    }
                }
            }
