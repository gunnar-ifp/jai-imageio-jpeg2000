    ...
    implements BlkImgDataSrc
{
     ...
     *
     * <P>
     * This default implementation assumes that the number of
    ...
    public static int[] calcMixedBitDepths(int ntdepth[], int ttype, int tdepth[])
    {
        ...
        switch (ttype) {
            ...
            case FORW_ICT:
                ...
                // use 'MathUtil.log2(2*x-1)+1', which calculates ceil(log2(x))
                // for any x>=1, x integer.
                tdepth[0] = MathUtil.log2((int)Math.floor((1 << ntdepth[0]) * 0.299072 +
                    (1 << ntdepth[1]) * 0.586914 +
                    (1 << ntdepth[2]) * 0.114014) - 1) + 1;
                tdepth[1] = MathUtil.log2((int)Math.floor((1 << ntdepth[0]) * 0.168701 +
                    (1 << ntdepth[1]) * 0.331299 +
                    (1 << ntdepth[2]) * 0.5) - 1) + 1;
                tdepth[2] = MathUtil.log2((int)Math.floor((1 << ntdepth[0]) * 0.5 +
